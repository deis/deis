...
.. code-block:: console
    ...
    1.11.0
    $ deisctl config platform set version=v1.11.0
...
from 1.9.0 to 1.9.1 or 1.11.0). Upgrading between major versions is not supported (for example, from 1.9.0 to a
...
.. code-block:: console
    ...
    $ mkdir /tmp/upgrade
    $ curl -sSL http://deis.io/deisctl/install.sh | sh -s 1.11.0 /tmp/upgrade
    $ /tmp/upgrade/deisctl --version  # should match the desired platform
    1.11.0
    $ /tmp/upgrade/deisctl refresh-units
    $ /tmp/upgrade/deisctl config platform set version=v1.11.0

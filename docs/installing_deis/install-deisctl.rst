...
.. code-block:: console
    ...
    $ cd ~/bin
    $ curl -sSL http://deis.io/deisctl/install.sh | sh -s 1.8.0
    $ # on CoreOS, add "sudo" to install to /opt/bin/deisctl
    $ curl -sSL http://deis.io/deisctl/install.sh | sudo sh -s 1.8.0

This installs ``deisctl`` version 1.8.0 to the current directory, and downloads the matching
...
.. image:: download-linux-brightgreen.svg
   :target: https://s3-us-west-2.amazonaws.com/opdemand/deisctl-1.8.0-linux-amd64.run

.. image:: download-osx-brightgreen.svg
   :target: https://s3-us-west-2.amazonaws.com/opdemand/deisctl-1.8.0-darwin-amd64.run
